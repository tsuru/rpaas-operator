...
var debugH2cClient = &http.Client{
...
func Test_Debug(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			request: func(t *testing.T, serverURL string) {
				uri := fmt.Sprintf("ws://%s/resources/my-instance/debug?ws=true&command=bash&tty=true&width=80&height=24", strings.TrimPrefix(serverURL, "http://"))
		...
		},
		{
			...
			request: func(t *testing.T, serverURL string) {
				uri := fmt.Sprintf("ws://%s/resources/my-instance/debug?ws=true&interactive=true&command=bash&tty=true&width=80&height=24&image=tsuru/debug", strings.TrimPrefix(serverURL, "http://"))
			...
			manager: &fake.RpaasManager{
				FakeDebug: func(instance string, args rpaas.DebugArgs) error {
					called = true
					...
					assert.Equal(t, args.Image, "tsuru/debug")
		...
		},
		{
			...
			request: func(t *testing.T, serverURL string) {
				pr, pw := io.Pipe()
				uri := fmt.Sprintf("%s/resources/my-instance/debug?ws=false&interactive=true&command=/bin/sh&tty=true&width=124&height=80&image=tsuru/debug", serverURL)
				request, err := http.NewRequest("POST", uri, pr)
				require.NoError(t, err)

				response, err := debugH2cClient.Do(request)
				require.NoError(t, err)
		...
		},
		{
			...
			request: func(t *testing.T, serverURL string) {
				uri := fmt.Sprintf("%s/resources/my-instance/debug?command=some-command", serverURL)
