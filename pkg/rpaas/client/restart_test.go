...
func TestClientThroughTsuru_Restart(t *testing.T) {
	tests := []struct {
		name          string
		instance      string
		...
		},
		{
			...
			handler: func(w http.ResponseWriter, r *http.Request) {
				...
				assert.Equal(t, fmt.Sprintf("/1.20/services/%s/resources/%s/restart", FakeTsuruService, "my-instance"), r.URL.RequestURI())
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			defer server.Close()
			err := client.Restart(context.TODO(), tt.instance)
			if tt.expectedError == "" {
