...
`,
		Flags: []cli.Flag{
			...
			&cli.IntFlag{
				Name:        "sdsw",
				Aliases:     []string{"scale-down-stabilization-window-seconds"},
				Usage:       "is the number of seconds for which past recommendations should be considered while scaling up or scaling down",
				DefaultText: "N/A",
			},
			&cli.IntFlag{
				Name:        "sdu",
				Aliases:     []string{"scale-down-units-policy-value"},
				Usage:       "set a policy with (value: unitsPolicyValue, type: Pods, periodSeconds: 60s)",
				DefaultText: "N/A",
			},
			&cli.IntFlag{
				Name:        "sdp",
				Aliases:     []string{"scale-down-percent-policy-value"},
				Usage:       "set a policy with (value: percentPolicyValue, type: Percent, periodSeconds: 60s)",
				DefaultText: "N/A",
			},
		},
...
func runUpdateAutoscale(ctx context.Context, cmd *cli.Command) error {
	...
	if cmd.IsSet("sdsw") {
		n := cmd.Int("sdsw")
		if n <= 0 {
			return fmt.Errorf("sdsw must be greater than zero")
		}
		sdsw = autogenerated.PtrInt32(int32(n))
	}
	...
	if cmd.IsSet("sdu") {
		n := cmd.Int("sdu")
		if n <= 0 {
			return fmt.Errorf("sdu must be greater than zero")
		}
		sdu = autogenerated.PtrInt32(int32(n))
	}
	...
	if cmd.IsSet("sdp") {
		n := cmd.Int("sdp")
		if n <= 0 {
			return fmt.Errorf("sdp must be greater than zero")
		}
		sdp = autogenerated.PtrInt32(int32(n))
	}
	...
	autoscale := autogenerated.Autoscale{
		...
		Behavior: &autogenerated.Behavior{
			ScaleDown: &autogenerated.ScaleDown{
				UnitsPolicyValue:           sdu,
				PercentPolicyValue:         sdp,
				StabilizationWindowSeconds: sdsw,
			},
		},
	}
...
func writeAutoscale(w io.Writer, autoscale *autogenerated.Autoscale) {
	...
	table := newTable(w)
	table.Header("Triggers", "trigger details")

	if autoscale.Cpu != nil {
		table.Append("CPU", fmt.Sprintf("%d%%", int(*autoscale.Cpu)))
	}

	if autoscale.Memory != nil {
		table.Append("Memory", fmt.Sprintf("%d%%", int(*autoscale.Memory)))
	}

	if autoscale.Rps != nil {
		table.Append("RPS", fmt.Sprintf("%d req/s", int(*autoscale.Rps)))
	}

	if b := autoscale.Behavior; b != nil && b.ScaleDown != nil {
		scaleDownStr := ""
		if b.ScaleDown.UnitsPolicyValue != nil {
			scaleDownStr += fmt.Sprintf("Units: %d\n", *b.ScaleDown.UnitsPolicyValue)
		}
		if b.ScaleDown.PercentPolicyValue != nil {
			scaleDownStr += fmt.Sprintf("Percentage: %d%%\n", *b.ScaleDown.PercentPolicyValue)
		}
		if b.ScaleDown.StabilizationWindowSeconds != nil {
			scaleDownStr += fmt.Sprintf("Stabilization window: %ds\n", *b.ScaleDown.StabilizationWindowSeconds)
		}
		table.Options(tablewriter.WithRendition(tw.Rendition{
			Settings: tw.Settings{Separators: tw.Separators{BetweenRows: tw.On}},
		}))
		table.Append("Scale down behavior", strings.TrimSuffix(scaleDownStr, "\n"))
	}
