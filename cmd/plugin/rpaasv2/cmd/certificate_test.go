...
func TestPostCertificate(t *testing.T) {
	testCases := []struct {
		...
		assertion func(t *testing.T, err error, body string)
	}{
		{
...
-----END EC PRIVATE KEY-----`, string(keyBytes))
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			tt.cert.prox = proxy.New(tt.cert.service, tt.cert.instance, "POST", &mockServer{ts: ts})
			tt.cert.prox.Path = "/resources/test-instance/certificate"
			defer ts.Close()
			body, boundary, err := mockEncodeBody()
			assert.NilError(t, err)
			err = postCertificate(tt.cert.prox, body, boundary)
			assert.NilError(t, err)
		})
	}
}
