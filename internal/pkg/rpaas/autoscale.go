...
func validateAutoscale(a *autogenerated.Autoscale) error {
	...
	if b := a.Behavior; b != nil && b.ScaleDown != nil {
		if unitsPolicyValue := b.ScaleDown.UnitsPolicyValue; unitsPolicyValue != nil && *unitsPolicyValue <= 0 {
			return &ValidationError{Msg: "units policy value must be greater than zero"}
		}

		if percentPolicyValue := b.ScaleDown.PercentPolicyValue; percentPolicyValue != nil && *percentPolicyValue <= 0 {
			return &ValidationError{Msg: "percent policy value must be greater than zero"}
		}

		if stabilizationWindowSeconds := b.ScaleDown.StabilizationWindowSeconds; stabilizationWindowSeconds != nil && *stabilizationWindowSeconds <= 0 {
			return &ValidationError{Msg: "stabilization window seconds must be greater than zero"}
		}
	}
